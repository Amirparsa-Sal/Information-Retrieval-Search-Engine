...
def fill_column_numbers(sheet):
    columns_dic = dict()

    for i in range (1,5):
        columns_dic[sheet.cell(row=1, column=i).value] = i

    if 'title' not in columns_dic:
        columns_dic['title'] = columns_dic['url']
...
def count_tokens_and_text_length(sheet, n, content_column, stemming=True):
    ...
    while True:
        ...
        if len(query) != 0:
            ...
            if news is None:
                ...
                print('News found:')
                news = list(map(lambda doc_id: (sheet.cell(row=int(doc_id)+1, column=columns_dic['url']).value, sheet.cell(row=int(doc_id)+1, column=columns_dic['title']).value, doc_id), news))
